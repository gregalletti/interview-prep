...
This pattern is perfect for removing duplicates from sorted arrays or partitioning data without extra space.
...
- **Sorted array:** two pointers is often the first thing to try.
- **"In-place" or "constant extra space":** two pointers is a strong signal.
- **Linked lists:** fast-slow pointers are usually the right pattern.
